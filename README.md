...
| Données           | FAISS local uniquement | FAISS + fallback smolagents (web temps réel) |
| Sources web       | Aucune | Sites ciblés (infolocale, ticketmaster, fnac...) |
...
graph TD
    ...
    classDef user stroke:#2E86C1,stroke-width:2px,fill:#EBF5FB,color:#1B4F72;
    classDef pipeline stroke:#D4AC0D,stroke-width:2px,fill:#FEF9E7,color:#7D6608;
    classDef fallback stroke:#CB4335,stroke-width:2px,fill:#FADBD8,color:#78281F;
    classDef support stroke:#27AE60,stroke-width:2px,fill:#EAFAF1,color:#145A32;

    %% Nœuds de l'architecture
    USR[👤 UTILISATEUR<br/>Question + Ville sélectionnée]:::user

    subgraph PIPELINE RAG HYBRIDE
        FAISS[🔍 FAISS MMR<br/>local]:::pipeline
        COND{📊 Nombre de<br/>résultats ?}:::pipeline
        MISTRAL[✍️ Mistral-7B<br/>génération]:::pipeline
    end

    subgraph FALLBACK SMOLAGENTS
        AGENT[🌐 Agent Web]:::fallback
        DDG[🕷️ DuckDuckGo → Sites ciblés<br/>infolocale.fr, ticketmaster.fr, fnacspectacles.com...]:::fallback
        FORMAT[🧠 Extraction & Formatage<br/>Mistral-7B]:::fallback
    end

    subgraph SYSTÈMES SUPPORT
        MEM[💾 MÉMOIRE CONVERSATIONNELLE<br/>session_state, window=10]:::support
        MONITOR[📈 MONITORING<br/>latence, FAISS vs Agent, nb requêtes]:::support
    end
